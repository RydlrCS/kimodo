...
class FireworksPlannerClient:
    """Fireworks Inference API client using OpenAI-compatible chat completions."""
    ...
    def text_generation(self, prompt: str, model: str, max_new_tokens: int, temperature: float) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"fireworks network error: {exc}") from exc

        choices = data.get("choices") if isinstance(data, dict) else None
        if not isinstance(choices, list) or not choices:
            raise RuntimeError("fireworks response missing choices")
        message = choices[0].get("message") if isinstance(choices[0], dict) else None
        content = message.get("content") if isinstance(message, dict) else None
        if not isinstance(content, str) or not content.strip():
            raise RuntimeError("fireworks response missing content")
        return content
...
class PlannerConfig:
    ...
    def from_env(cls) -> PlannerConfig:
        raw_models = (os.getenv("KIMODO_PLANNER_MODELS") or "").strip()
        model_candidates = tuple(m.strip() for m in raw_models.split(",") if m.strip())
        if not model_candidates:
            model_candidates = cls.model_candidates
        return cls(model_candidates=model_candidates)
...
class QwenPlannerAdapter:
    ...
    def _build_default_client(self) -> PlannerClient:
        provider = (os.getenv("KIMODO_PLANNER_PROVIDER") or "hf").strip().lower()
        if provider == "fireworks":
            api_key = os.getenv("FIREWORKS_API_KEY")
            if not api_key:
                raise ValueError("FIREWORKS_API_KEY is required when KIMODO_PLANNER_PROVIDER=fireworks")
            base_url = os.getenv("FIREWORKS_BASE_URL") or "https://api.fireworks.ai/inference/v1"
            return FireworksPlannerClient(api_key=api_key, base_url=base_url)
        return HFInferencePlannerClient(token=os.getenv("HF_TOKEN"))
